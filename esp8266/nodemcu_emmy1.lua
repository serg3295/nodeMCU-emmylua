...
---@return mcp17 @instance for mcp23017
---`error` if MCP23017 address is out of range or device MCP23017 on address not found.
...
---@return fObj fileobject @if file opened ok. `nil` if file not opened, or not exists (read modes).
...
---@param SPIFFs_only? any @(optional) if not `nil` LFS files won't be included in the result (LFS files are returned only if the parameter is `nil`)
---@return table @a Lua table which contains all {file name: file size} pairs, if no pattern given.
...
---@param oldname string @old file name
---@param newname string @new file name
---@return boolean @`true` on success, `false` when the file is stored in LFS (so read-only) or on error
...
---@return fObj LFSfileobject @LFS file object (Lua table) or SPIFFS file object if file opened ok. `nil` if file not opened, or not exists (read modes).
